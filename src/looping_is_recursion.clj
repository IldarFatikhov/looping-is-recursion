...
(defn power [base exp]
  (let [pwr (fn [i acc] (if (zero? i) acc (recur (dec i) (* acc base))))]
  (pwr exp 1)))

(defn last-element [a-seq]
  (if (or (= 1 (count a-seq)) (= 0 (count a-seq)))
    (first a-seq)
    (recur (rest a-seq))))

(defn seq= [seq1 seq2]
  (if (or (empty? seq1) (empty? seq2))
    (if (and (empty? seq1) (empty? seq2)) true false)
...
(defn avg [a-seq]
  (loop [acc 0 sum 0 seq a-seq]
    (cond
      (empty? seq)
        (/ sum acc)
      :else (recur (inc acc) (+ sum (first seq)) (rest seq)))))
...
(defn cut-at-repetition [a-seq]
  (loop [loop-seq a-seq
         a-set #{}
         return-seq ()]
    (if (empty? loop-seq) (reverse return-seq)
